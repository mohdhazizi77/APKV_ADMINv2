...
Public Class import_markah_matapelajaran
    ...
    Private Sub kpmkv_kodkursus_list()
        ...
        strSQL += " WHERE kpmkv_kelas.KolejRecordID='" & oCommon.FixSingleQuotes(ddlKolej.Text) & "' AND kpmkv_kursus.Tahun='" & ddlTahun.Text & "' AND kpmkv_kursus.Sesi='" & chkSesi.SelectedValue & "' GROUP BY kpmkv_kursus.KodKursus, kpmkv_kursus.KursusID"
        Dim strConn As String = ConfigurationManager.AppSettings("ConnectionString")
        Dim objConn As SqlConnection = New SqlConnection(strConn)
        Dim sqlDA As New SqlDataAdapter(strSQL, objConn)
            ...
            For i = 0 To SiteData.Tables(0).Rows.Count - 1
                ...
                If Not String.IsNullOrEmpty(strPelajarID) Then
                    ...
                    If chkResult.Text = "PB" Then
                        ...
                        strSQL += " WHERE KolejRecordID='" & ddlKolej.Text & "' AND PelajarID='" & oCommon.FixSingleQuotes(strPelajarID) & "'"
