...
Public Class kelas_pensyarah_akademik1
    ...
    Private Sub kpmkv_namakluster_list()
        ...
        strSQL += " WHERE kpmkv_kursus.Tahun='" & ddlTahun.Text & "' AND  kpmkv_kursus.Sesi='" & chkSesi.Text & "' AND kpmkv_kursus_kolej.KolejRecordID='" & ddlKolej.SelectedValue & "' GROUP BY kpmkv_kluster.NamaKluster,kpmkv_kluster.KlusterID"
    ...
    Private Sub kpmkv_kodkursus_list()
        ...
        strSQL += " ON kpmkv_kursus_kolej.KursusID = kpmkv_kursus.KursusID LEFT OUTER JOIN kpmkv_kluster "
        strSQL += " ON kpmkv_kursus.KlusterID = kpmkv_kluster.KlusterID"
        strSQL += " WHERE kpmkv_kursus_kolej.KolejRecordID='" & ddlKolej.Text & "' AND kpmkv_kursus.Tahun='" & ddlTahun.Text & "' "
        strSQL += " AND kpmkv_kursus.Sesi='" & chkSesi.SelectedValue & "'  AND kpmkv_kluster.KlusterID='" & ddlNamaKluster.Text & "' "
    ...
    Private Sub kpmkv_kelas_list()
        ...
        strSQL += " WHERE kpmkv_kelas.KolejRecordID='" & ddlKolej.Text & "'  AND kpmkv_kelas.Tahun= '" & ddlTahun.Text & "' "
        strSQL += " AND kpmkv_kelas.IsDeleted='N' AND kpmkv_kelas_kursus.KursusID='" & ddlKodKursus.Text & "'"
        'Response.Write(strSQL)
        Dim strConn As String = ConfigurationManager.AppSettings("ConnectionString")
        Dim objConn As SqlConnection = New SqlConnection(strConn)
        Dim sqlDA As New SqlDataAdapter(strSQL, objConn)
    ...
    Private Function getSQL() As String
        ...
        Dim strOrder As String = " ORDER BY Tahun,KodMataPelajaran ASC"

        tmpSQL = " SELECT * FROM  kpmkv_matapelajaran "
        strWhere = "WHERE SUBSTRING(KodMataPelajaran,4,1) ='" & ddlSemester.Text & "' AND Tahun='" & ddlTahun.Text & "' AND IsDeleted='N'"
    ...
    Private Function getSQL2() As String
        ...
        strWhere = "  WHERE KolejRecordId='" & ddlKolej.SelectedValue & "' AND IsDeleted='N'"
    ...
    Private Sub ddlNamaKelas_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlNamaKelas.SelectedIndexChanged
        kpmkv_kelas_list()
        ddlNamaKelas.SelectedValue = 0
    End Sub
End Class