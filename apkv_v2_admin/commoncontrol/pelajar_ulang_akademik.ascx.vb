...
Public Class pelajar_ulang_akademik
    ...
    Private Sub kpmkv_kodkursus_list()
        ...
        strSQL += " WHERE kpmkv_kursus_kolej.KolejRecordID='" & ddlKolej.SelectedValue & "' AND kpmkv_kursus.Tahun='" & ddlTahun.SelectedValue & "' AND kpmkv_kursus.Sesi='" & chkSesi.SelectedValue & "'"
        Dim strConn As String = ConfigurationManager.AppSettings("ConnectionString")
        Dim objConn As SqlConnection = New SqlConnection(strConn)
        Dim sqlDA As New SqlDataAdapter(strSQL, objConn)
    ...
    Private Function getSQL() As String
        ...
        strWhere = " WHERE kpmkv_pelajar.IsDeleted='N' AND kpmkv_pelajar.StatusID='2' AND kpmkv_pelajar_markah.KolejRecordID='" & ddlKolej.SelectedValue & "'"
        strWhere += " AND (kpmkv_pelajar_markah.GredBM IN ('D','E','D-','D+','T') "
        strWhere += " OR kpmkv_pelajar_markah.GredBI IN ('D','E','D-','D+','T')"
        strWhere += " OR kpmkv_pelajar_markah.GredSC IN ('D','E','D-','D+','T') "
        strWhere += " OR  kpmkv_pelajar_markah.GredSJ IN ('D','E','D-','D+','T')"
        strWhere += " OR kpmkv_pelajar_markah.GredPI IN ('D','E','D-','D+','T') "
        strWhere += " OR  kpmkv_pelajar_markah.GredPM IN ('D','E','D-','D+','T')"
        strWhere += " OR kpmkv_pelajar_markah.GredMT IN ('D','E','D-','D+','T') )"
