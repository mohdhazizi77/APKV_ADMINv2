...
Public Class gred_sjsetara_update1
    ...
    Private Sub LoadPage()
        strSQL = "SELECT * FROM kpmkv_gred_sejarah WHERE GSID='" & Request.QueryString("GSID") & "'"
        Dim strConn As String = ConfigurationManager.AppSettings("ConnectionString")
        Dim objConn As SqlConnection = New SqlConnection(strConn)
        Dim sqlDA As New SqlDataAdapter(strSQL, objConn)
    ...
    Private Sub btnUpdate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        ...
        Try
            ...
            '--execute
            If kpmkv_gred_sejarah_update() = True Then
                divMsg.Attributes("class") = "info"
                lblMsg.Text = "Berjaya mengemaskini maklumat Gred SJ Setara ."
            Else
                divMsg.Attributes("class") = "error"
            End If
    ...
    Private Function ValidatePage() As Boolean
        ...
        If Not lblGred.Text = txtGred.Text Then       '--changes made to the Gred
            strSQL = "SELECT Gred FROM kpmkv_gred_sejarah WHERE Gred='" & oCommon.FixSingleQuotes(txtGred.Text) & "'"
        ...
        Return True
    End Function
    Private Function kpmkv_gred_sejarah_update() As Boolean
        strSQL = "UPDATE kpmkv_gred_sejarah SET Tahun='" & oCommon.FixSingleQuotes(ddlTahun.Text) & "',Sesi='" & oCommon.FixSingleQuotes(chkSesi.Text) & "',MarkahFrom='" & oCommon.FixSingleQuotes(txtMarkah.Text) & "',MarkahTo='" & oCommon.FixSingleQuotes(txtMarkahTo.Text) & "',Gred='" & oCommon.FixSingleQuotes(txtGred.Text.ToUpper) & "',Status='" & oCommon.FixSingleQuotes(txtStatus.Text.ToUpper) & "',Kompetensi='" & oCommon.FixSingleQuotes(txtKompetensi.Text.ToUpper) & "' WHERE GSID='" & Request.QueryString("GSID") & "'"
    ...
    Private Sub btnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        strSQL = "DELETE kpmkv_gred_sejarah WHERE GSID='" & Request.QueryString("GSID") & "'"
        strRet = oCommon.ExecuteSQL(strSQL)
        If strRet = "0" Then
    ...
    Protected Sub btlist_Click(sender As Object, e As EventArgs) Handles btlist.Click
        Response.Redirect("gred.sjsetara.list.aspx")
    End Sub
End Class