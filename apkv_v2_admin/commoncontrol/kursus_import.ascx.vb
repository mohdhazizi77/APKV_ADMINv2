...
Public Class kursus_import
    ...
    Private Function getSQL() As String
        ...
        Dim strOrder As String = " ORDER BY NamaKursus ASC"

        tmpSQL = "SELECT * FROM kpmkv_kursus_txn"
    ...
    Protected Function ValidateSiteData(ByVal SiteData As DataSet) As String
        ...
        Try
            ...
            '        '--NamaKluster is required!
            '        If Not IsDBNull(SiteData.Tables(0).Rows(i).Item("NamaKluster")) Then
            '            strNamaKluster = SiteData.Tables(0).Rows(i).Item("NamaKluster")
            '        Else
            '            sb.Append("Nama:" & strNamaKluster & ". NamaKluster# is required.<br />")
            '        End If

            '        '--strKodKursus is required!
            '        If Not IsDBNull(SiteData.Tables(0).Rows(i).Item("KodKursus")) Then
            '            strKodKursus = SiteData.Tables(0).Rows(i).Item("KodKursus")
            '        Else
            '            sb.Append("Nama:" & strKodKursus & ". KodKursus# is required.<br />")
            '        End If

            '        '--strNamaKursus is required!
            '        If Not IsDBNull(SiteData.Tables(0).Rows(i).Item("NamaKursus")) Then
            '            strNamaKursus = SiteData.Tables(0).Rows(i).Item("NamaKursus")
            '        Else
            '            sb.Append("Nama:" & strNamaKursus & ". NamaKursus# is required.<br />")
            '        End If
            '    Next
    ...
    Protected Function ProcessSiteImportFile(ByVal SiteData As DataSet) As String
        ...
        Dim command As SqlCommand = New SqlCommand("INSERT INTO kpmkv_kursus_txn(Tahun,Semester,Sesi,NamaKluster,KodKursus,NamaKursus,KodModul,NamaModul,JamKredit,IsDeleted) " _
        ...
        Try
            For i As Integer = 0 To SiteData.Tables(0).Rows.Count - 1
                ...
                command.Parameters.AddWithValue("@Tahun", Left(SiteData.Tables(0).Rows(i).Item("Tahun").ToString(), 4))
                command.Parameters.AddWithValue("@Semester", Left(SiteData.Tables(0).Rows(i).Item("Semester").ToString(), 1))
                command.Parameters.AddWithValue("@Sesi", Left(SiteData.Tables(0).Rows(i).Item("Sesi").ToString(), 1))
                command.Parameters.AddWithValue("@NamaKluster", Left(SiteData.Tables(0).Rows(i).Item("NamaKluster").ToString(), 255))
                command.Parameters.AddWithValue("@KodKursus", Left(SiteData.Tables(0).Rows(i).Item("KodKursus").ToString(), 10))
                command.Parameters.AddWithValue("@NamaKursus", Left(SiteData.Tables(0).Rows(i).Item("NamaKursus").ToString(), 255))
                command.Parameters.AddWithValue("@KodModul", Left(SiteData.Tables(0).Rows(i).Item("KodModul").ToString(), 10))
    ...
    Private Sub datRespondent_SelectedIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewSelectEventArgs) Handles datRespondent.SelectedIndexChanging
        ...
        ' Response.Redirect("kursus.update.aspx?RecordID=" & strKeyID)
        ...
        For i = 0 To datRespondent.Rows.Count - 1 Step i + 1
            ...
            If Not chkUpdate Is Nothing Then
                If chkUpdate.Checked = True Then
                    ...
                    strSQL = "UPDATE kpmkv_kursus_txn SET IsDeleted='Y' WHERE TxnKursusID='" & strID & "'"
    ...
    Protected Sub lnkList_Click(ByVal sender As Object, ByVal e As EventArgs) Handles lnkList.Click
        ' Response.Redirect("kursus.list.aspx")
