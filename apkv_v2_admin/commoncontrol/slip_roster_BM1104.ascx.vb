...
Public Class slip_roster_BM11041
            ...
            For listNegeri As Integer = 0 To dsNegeri.Tables(0).Rows.Count - 1
                ...
                For i As Integer = 0 To dsRecordID.Tables(0).Rows.Count - 1
                    ...
                    For j As Integer = 0 To dsPelajarID.Tables(0).Rows.Count - 1
                        ...
                        Dim strGredBM As String = ""
                        ''--Tokenid,ClassCode,Q001Remarks
                        strSQL = "SELECT Nama FROM kpmkv_pelajar WHERE PelajarID='" & strKey & "'"
                        strNama = oCommon.getFieldValue(strSQL)

                        strSQL = "SELECT Mykad FROM kpmkv_pelajar WHERE PelajarID='" & strKey & "'"
                        strMykad = oCommon.getFieldValue(strSQL)

                        strSQL = "SELECT AngkaGiliran FROM kpmkv_pelajar WHERE PelajarID='" & strKey & "'"
                        strAngkaGiliran = oCommon.getFieldValue(strSQL)
                        ...
                        ''--Pointer BM 
                        'strSQL = "SELECT PointerBMSetara FROM kpmkv_pelajar_markah WHERE PelajarID='" & datRespondent.DataKeys(i).Value.ToString & "'"
                        strPointer = oCommon.getFieldValue("SELECT PointerBMSetara FROM kpmkv_pelajar_markah WHERE PelajarID='" & strKey & "'")
                        ''--Gred BM 
                        If strPointer = "-1" Then
                            strGredBM = "T"
                        ElseIf strPointer = "" Then
                        ...
                        'change on 17082016
                        strSQL = "UPDATE kpmkv_pelajar_markah SET GredBMSetara='" & strGredBM & "' Where PelajarID='" & strKey & "'"
                        strRet = oCommon.ExecuteSQL(strSQL)
