...
Public Class kelas_modul_pensyarah
    ...
    Private Sub kpmkv_namakluster_list()
        ...
        strSQL += " WHERE kpmkv_kursus.Tahun='" & ddlTahun.SelectedValue & "' AND  kpmkv_kursus.Sesi='" & chkSesi.SelectedValue & "' AND kpmkv_kursus_kolej.KolejRecordID='" & ddlKolej.SelectedValue & "' GROUP BY kpmkv_kluster.NamaKluster,kpmkv_kluster.KlusterID"
    ...
    Private Sub kpmkv_kodkursus_list()
        ...
        strSQL += " ON kpmkv_kursus_kolej.KursusID = kpmkv_kursus.KursusID LEFT OUTER JOIN kpmkv_kluster "
        strSQL += " ON kpmkv_kursus.KlusterID = kpmkv_kluster.KlusterID"
        strSQL += " WHERE kpmkv_kursus_kolej.KolejRecordID='" & ddlKolej.Text & "' AND kpmkv_kursus.Tahun='" & ddlTahun.Text & "' AND kpmkv_kursus.Sesi='" & chkSesi.Text & "'  AND kpmkv_kluster.KlusterID='" & ddlNamaKluster.Text & "' GROUP BY kpmkv_kursus.KodKursus, kpmkv_kursus.KursusID"
    ...
    Private Sub kpmkv_kelas_list()
        ...
        strSQL += " WHERE kpmkv_kelas.KolejRecordID='" & ddlKolej.Text & "'  AND kpmkv_kelas.Tahun= '" & ddlTahun.Text & "' AND kpmkv_kelas.IsDeleted='N' AND kpmkv_kelas_kursus.KursusID='" & ddlKodKursus.Text & "'"
        'Response.Write(strSQL)
        Dim strConn As String = ConfigurationManager.AppSettings("ConnectionString")
        Dim objConn As SqlConnection = New SqlConnection(strConn)
        Dim sqlDA As New SqlDataAdapter(strSQL, objConn)
    ...
    Private Function getSQL() As String
        ...
        Dim strOrder As String = " ORDER BY Tahun,Semester,Sesi,KursusID"
    ...
    Private Function getSQL2() As String
        ...
        strWhere = "  WHERE KolejRecordId='" & ddlKolej.Text & "' AND IsDeleted='N'"
    ...
    Protected Sub datRespondent2_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles datRespondent2.RowCommand
        If (e.CommandName = "Pilih") Then
            ...
            For i As Integer = 0 To datRespondent.Rows.Count - 1
                ...
                If isChecked Then
                    strSQL = "SELECT * FROM kpmkv_pensyarah_modul WHERE KelasID='" & ddlNamaKelas.Text & "' And KursusID='" & ddlKodKursus.Text & "' And ModulID='" & str & "' "
                    strRet = oCommon.isExist(strSQL)
                    If strRet = True Then

                        strSQL = "DELETE FROM kpmkv_pensyarah_modul WHERE KelasID='" & ddlNamaKelas.Text & "' And KursusID='" & ddlKodKursus.Text & "' And ModulID='" & str & "' "
                        strRet = oCommon.getFieldValue(strSQL)
                    End If

                    strSQL = "INSERT INTO kpmkv_pensyarah_modul(PensyarahID, KelasID, KursusID, ModulID, IsDeleted)"
                    strSQL += " VALUES('" & PensyarahID & "', '" & ddlNamaKelas.Text & "', '" & ddlKodKursus.Text & "', '" & str & "', 'N')"
    ...
    Protected Sub ddlJenis_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlJenis.SelectedIndexChanged
        ...
        kpmkv_namakluster_list()
        ddlNamaKluster.SelectedValue = 0
