...
Public Class pentaksiran_sijil_vokasional
    ...
    Private Sub kpmkv_kolej_list()
        strSQL = "SELECT Nama,RecordID FROM kpmkv_kolej WHERE Negeri='" & ddlNegeri.SelectedItem.Text & "' "
        strSQL += " And Jenis='" & ddlJenis.SelectedItem.Text & "' ORDER BY Nama"
        Dim strConn As String = ConfigurationManager.AppSettings("ConnectionString")
        Dim objConn As SqlConnection = New SqlConnection(strConn)
        Dim sqlDA As New SqlDataAdapter(strSQL, objConn)
    ...
    Private Sub kpmkv_kelas_list()
        ...
        strSQL += " WHERE kpmkv_kelas.KolejRecordID='" & ddlKolej.Text & "' AND kpmkv_kelas_kursus.KursusID= '" & ddlKodKursus.Text & "' AND kpmkv_kursus.Tahun= '" & ddlTahun.Text & "' ORDER BY  kpmkv_kelas.NamaKelas"
        Dim strConn As String = ConfigurationManager.AppSettings("ConnectionString")
        Dim objConn As SqlConnection = New SqlConnection(strConn)
        Dim sqlDA As New SqlDataAdapter(strSQL, objConn)
        ...
        Try
            ...
            For i As Integer = 0 To datRespondent.Rows.Count - 1
                ...
                If cb.Checked = True Then
                    ...
                    strSQL = "SELECT JamKredit FROM kpmkv_matapelajaran WHERE SUBSTRING(KodMataPelajaran,4,1)='4' AND NamaMataPelajaran='SCIENCE' AND Tahun='" & ddlTahun.Text & "'"
                    strRet = oCommon.getFieldValueEx(strSQL)
